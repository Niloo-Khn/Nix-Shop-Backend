...
test("account API registers and authenticates through HTTP", async (context) => {
  ...
  const registration = await fetch(`${service.baseUrl}/accounts/register`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({email:"pet@example.com",firstName:"Pet",familyName:"Owner",phone:"+1 555 123 4567",password:"long-test-password"}) });
  ...
  const update=await fetch(`${service.baseUrl}/accounts/me`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:authorization},body:JSON.stringify({firstName:"Pet",familyName:"Parent",phone:"+1 555 123 4567",address:"1 Pet Street",birthday:"1990-05-12"})});
