...
test("order service includes shipping in the trusted total",async()=>{const order=await new OrderService(new InMemoryRepository<Order>(),catalog).create({items:[{productId:"toy",quantity:1}]},"account-1");assert.equal(order.subtotal,5);assert.equal(order.shipping,6);assert.equal(order.amount,11);});
