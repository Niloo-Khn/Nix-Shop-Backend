...
export class ProductService {
  ...
  async get(id: string): Promise<Product> { const product = await this.repository.find(id); if (!product?.active) throw new HttpError(404, ERROR_KEYS.productNotFound); return product; }
  async create(input: unknown): Promise<Product> { const body = requireFields(input,["sku"]); const price = Number(body.price); if (!Number.isFinite(price) || price < 0) throw new HttpError(400,ERROR_KEYS.priceInvalid); return this.repository.save({ id:randomUUID(),sku:String(body.sku).slice(0,64),price,active:true }); }
}
